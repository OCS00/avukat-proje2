// src/siteConfig.js
export const siteConfig = {
  // GENEL KİMLİK
  name: "Av. Ahmet Yılmaz", // Müşterinin adı buraya
  title: "Hukuk & Danışmanlık",
  description: "Ceza, Aile ve Ticaret hukuku alanında uzman avukatlık hizmetleri.",
  
  // İLETİŞİM (Burayı değiştirince her yer değişir)
  phone: "+90 (532) 123 45 67",
  email: "info@ahmetyilmaz.com",
  address: "Maslak Mah. Büyükdere Cad. No:123 Sarıyer/İstanbul",
  
  // HAKKIMDA KISMI
  experience: "15+", // Tecrübe yılı
  bioTitle: "Adalet arayışınızda güvenilir çözüm ortağınız.",
  shortBio: "2010 yılından bu yana İstanbul Barosu'na kayıtlı olarak, müvekkillerimin haklarını en üst seviyede savunmak için çalışıyorum.",
  
  // LİNKLER (Varsa doldur, yoksa # bırak)
  social: {
    instagram: "https://instagram.com",
    twitter: "https://twitter.com",
    linkedin: "https://linkedin.com",
  }
};